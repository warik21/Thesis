...
f, g, plan = unbalanced_sinkhorn(alpha, beta, C, eps, return_plan=True)


# Plots
# Plot target and source distributions
plot_distribution(X, alpha, beta, title='Source and target distributions')

# Direct output transport plan
# Plot results
plot_marginals(X, alpha, beta, plan, title='Source and target distributions')

# Plot transport plan with its marginals
plot_transport_map_with_marginals(X, Y, alpha, beta, plan, title='Transport matrix with its marginals')
